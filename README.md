...
> 截图补充中，请先通过 `python run.py` 启动程序体验。
...
- 内置 AI 聊天面板，支持多模型切换（DeepSeek、智谱 GLM 等）
- 流式输出、Markdown 渲染
- 兼容所有 OpenAI 格式 API
- 后续目标：集成到 CTA 策略中，结合行情数据和策略信号，为辅助交易提供 AI 分析建议

---

## 未来计划

- **CTA 策略接入 AI 分析** — 将 AI 服务集成到 CTA 策略流程中，结合行情数据和策略信号，自动生成交易分析建议，辅助人工决策
- **实时行情图表** — 基于 [lightweight-charts-python](https://github.com/louisnw01/lightweight-charts-python) 集成实时 K 线图表，展示行情走势、技术指标叠加和交易开平仓标记
